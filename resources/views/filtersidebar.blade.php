...
<head>
  ...
  <style>
    * {
    ...
    /* === PERBAIKAN STYLING UNTUK FILTER PANEL === */
    .filter-panel {
      background: white;
    ...
    /* TAMBAH: Backdrop untuk filter panel */
    .filter-backdrop {
      display: none;
      position: fixed;
      top: 0;
    ...
    .radius-slider {
      ...
      height: 8px;
      border-radius: 5px;
      background: linear-gradient(to right, #ED1C24 0%, #e9ecef 0%);
      outline: none;
    ...
    .category-chip.active {
      background: linear-gradient(135deg, #ED1C24, #c0392b);
      color: white;
      border-color: #ED1C24;
...
<body>
  ...
  <!-- Filter Panel - DIUBAH: Menjadi sidebar -->
...
<script>
  ...
  // Initialize filter functionality - DIUBAH
  function initFilter() {
    ...
    console.log('Initializing filter...', {
      ...
      categoryChips: categoryChips.length
    });

    // Toggle filter panel - DIUBAH: tambah backdrop
    filterToggle.addEventListener('click', (e) => {
      ...
      // Tambah class ke body untuk mencegah scroll
    ...
    // Close filter saat klik di backdrop - DIUBAH
    ...
    // Close filter saat klik di luar - DIUBAH
    ...
    // Prevent close ketika klik di dalam panel
    ...
    resetBtn.addEventListener('click', () => {
      ...
      if (radiusSlider) {
        ...
        if (radiusValue) radiusValue.textContent = '5';
        const percentage = ((5 - 0.5) / (5 - 0.5)) * 100;
        radiusSlider.style.background = `linear-gradient(to right, #ED1C24 0%, #ED1C24 ${percentage}%, #e9ecef ${percentage}%, #e9ecef 100%)`;
      }
    ...
    applyBtn.addEventListener('click', () => {
      ...
      currentRadius = parseFloat(radiusSlider ? radiusSlider.value : 5);
      const activeChip = document.querySelector('.category-chip.active');
      currentCategory = activeChip ? activeChip.dataset.category : 'all';
      
      console.log('Filter settings:', { currentRadius, currentCategory });
  ...
  function showFilterNotification() {
    ...
    notification.style.cssText = `
      ...
      animation: slideDown 0.3s ease-out;
    `;
    notification.innerHTML = `<i class="fas fa-check-circle"></i> Filter diterapkan (Radius: ${currentRadius}km, Kategori: ${getCategoryName(currentCategory)})`;
    document.body.appendChild(notification);
