...
<script>
  ...
  document.addEventListener('keydown', function(event) {
    ...
    // =============================================
// TRACKING REAL-TIME FUNCTIONS
// =============================================

let trackingInterval;
let lastUpdateTime = null;
let vendorPositions = new Map(); // Untuk menyimpan posisi terakhir vendor

// Fungsi untuk memulai tracking real-time
function startTracking() {
    ...
    // Jalankan tracking setiap 5 menit (300000 ms)
    trackingInterval = setInterval(() => {
        updateVendorLocations();
    }, 300000);
    
    console.log('Tracking real-time dimulai (setiap 5 menit)');
    
    // Jalankan sekali langsung saat mulai
    updateVendorLocations();
}

// Fungsi untuk memperbarui lokasi penjual
async function updateVendorLocations() {
    if (!map) return;
    
    // Tampilkan indikator update
    showUpdateIndicator();
    
    try {
        // Gunakan endpoint tracking yang di-cache untuk performa lebih baik
        const apiUrl = window.location.origin + '/api/vendors/locations/cached';
        console.log('Memperbarui lokasi penjual dari:', apiUrl);
        ...
        if (response.ok) {
            const result = await response.json();
            console.log('Data lokasi terbaru:', result);
            
            if (result.success && result.data) {
                // Perbarui posisi marker untuk setiap penjual
                updateMarkerPositions(result.data);
                
                // Perbarui waktu terakhir update
                updateLastUpdateTime();
                
                console.log(`Lokasi ${result.count} penjual berhasil diperbarui`);
            } else {
                console.error('Format response tidak valid:', result);
                // Fallback ke endpoint biasa
                await fallbackToRegularEndpoint();
            }
        } else {
            console.error('Gagal memperbarui lokasi:', response.status, response.statusText);
            // Fallback ke endpoint biasa
            await fallbackToRegularEndpoint();
        }
    } catch (error) {
        console.error('Error memperbarui lokasi:', error);
        // Fallback ke endpoint biasa
        await fallbackToRegularEndpoint();
    } finally {
        // Sembunyikan indikator update setelah 2 detik
        setTimeout(hideUpdateIndicator, 2000);
    }
}

// Fallback ke endpoint vendors biasa
async function fallbackToRegularEndpoint() {
    console.log('Fallback ke endpoint vendors biasa...');
    await loadVendors();
}

// Fungsi untuk memperbarui posisi marker di peta
function updateMarkerPositions(vendorsData) {
    let updatedCount = 0;
    let movedVendors = [];
    
    vendorsData.forEach(vendor => {
        // Cari marker yang sesuai dengan vendor
        const existingMarkerIndex = locationMarkers.findIndex(marker => {
            const markerVendor = allVendors.find(v => v.id === vendor.id);
            return markerVendor && markerVendor.id === vendor.id;
        });
        
        if (existingMarkerIndex !== -1) {
            const existingMarker = locationMarkers[existingMarkerIndex];
            const oldVendor = allVendors.find(v => v.id === vendor.id);
            
            // Periksa apakah lokasi berubah
            if (oldVendor && 
                (oldVendor.latitude !== vendor.latitude || oldVendor.longitude !== vendor.longitude)) {
                
                // Simpan posisi lama untuk animasi
                const oldPosition = existingMarker.getPosition();
                const newPosition = new google.maps.LatLng(vendor.latitude, vendor.longitude);
                
                // Update data vendor di allVendors
                const vendorIndex = allVendors.findIndex(v => v.id === vendor.id);
                if (vendorIndex !== -1) {
                    allVendors[vendorIndex] = { ...allVendors[vendorIndex], ...vendor };
                }
                
                // Animasikan perpindahan marker
                animateMarkerMovement(existingMarker, oldPosition, newPosition, () => {
                    console.log(`Marker ${vendor.nama} berhasil dipindahkan`);
                });
                
                updatedCount++;
                movedVendors.push(vendor.nama);
            }
        }
    });
    
    if (updatedCount > 0) {
        console.log(`${updatedCount} penjual berpindah lokasi:`, movedVendors);
        
        // Perbarui juga daftar di bottom sheet jika diperlukan
...
// Fungsi untuk animasi pergerakan marker
function animateMarkerMovement(marker, oldPosition, newPosition, onComplete) {
    const steps = 20; // Jumlah frame animasi
    const stepDelay = 50; // Delay per frame (ms)
    let currentStep = 0;
    ...
    if (marker.getElement()) {
        marker.getElement().classList.add('moving-marker');
    }
    
    const animateStep = () => {
        if (currentStep < steps) {
            const newLat = oldPosition.lat() + (latStep * currentStep);
            const newLng = oldPosition.lng() + (lngStep * currentStep);
            
            marker.setPosition(new google.maps.LatLng(newLat, newLng));
            currentStep++;
            setTimeout(animateStep, stepDelay);
        } else {
            // Pastikan marker berada di posisi akhir yang tepat
...
// Fungsi untuk menyembunyikan indikator update
...
function updateLastUpdateTime() {
    ...
    // Buat atau update elemen last update
    ...
    if (!lastUpdateEl) {
        ...
        lastUpdateEl.innerHTML = `Terakhir diperbarui: <span id="lastUpdateTime">${timeString}</span>`;
...
// Tambahkan CSS tracking ke document
const styleSheet = document.createElement('style');
styleSheet.textContent = trackingStyles;
document.head.appendChild(styleSheet);

// =============================================
// INITIALIZE TRACKING SAAT MAP LOAD
// =============================================

// Update initMap function untuk memulai tracking
const originalInitMap = initMap;
initMap = function() {
    originalInitMap();
    
    // Mulai tracking setelah map siap
    setTimeout(() => {
        startTracking();
    }, 2000);
};
...
// Fungsi untuk manual refresh (bisa dipanggil dari console)
window.manualRefresh = function() {
    updateVendorLocations();
};

// Fungsi untuk stop tracking
...
// Fungsi untuk start tracking manual
window.startTrackingManual = function() {
    startTracking();
};
  });
</script>
