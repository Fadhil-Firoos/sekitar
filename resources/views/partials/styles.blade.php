...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* PERBAIKAN: Contact actions dengan perbandingan 2:1 */
    .contact-actions {
      display: flex;
      gap: 10px;
    }
    ...
    /* PERBAIKAN: Button untuk layanan antar dengan perbandingan 1 */
    .delivery-btn {
      background: #3498db;
      color: white;
      border: none;
    ...
    /* PERBAIKAN: Modal untuk layanan antar */
    .delivery-modal {
      display: none;
      position: fixed;
      top: 0;
...
<body>
  ...
  <!-- Detail Page - PERBAIKAN: Hilangkan deskripsi & sesuaikan ukuran tombol -->
...
<script>
  ...
  function createVendorMarker(vendor, index) {
    ...
    const getCartIcon = (type, isOpen) => {
      const baseColor = isOpen ? '#e74c3c' : '#95a5a6';
    ...
    marker.addListener('click', () => {
      ...
      // PERBAIKAN: Hapus informasi layanan antar dari infoWindow
      infoWindow.setContent(`
        <div style="padding: 15px; max-width: 280px; font-family: 'Segoe UI', sans-serif;">
          <div style="display: flex; align-items: center; margin-bottom: 12px;">
            <div style="width: 8px; height: 8px; background: ${isOpen ? '#27ae60' : '#e74c3c'}; border-radius: 50%; margin-right: 8px;"></div>
            <h4 style="margin: 0; color: #2c3e50; font-size: 15px; flex: 1;">${vendor.nama}</h4>
            <span style="font-size: 10px; color: ${isOpen ? '#27ae60' : '#e74c3c'}; font-weight: bold;">
              ${isOpen ? 'BUKA' : 'TUTUP'}
            </span>
          </div>
          ...
          <div style="margin-bottom: 15px;">
            <div style="font-size: 11px; color: #7f8c8d; margin-bottom: 4px;">
              <i class="fas fa-clock" style="margin-right: 5px; color: #e74c3c;"></i>
              ${vendor.jam_buka} - ${vendor.jam_tutup}
            </div>
            <div style="font-size: 11px; color: #7f8c8d; margin-bottom: 4px;">
              <i class="fas fa-phone" style="margin-right: 5px; color: #e74c3c;"></i>
              +${vendor.whatsapp}
            </div>
            <div style="font-size: 11px; color: #7f8c8d;">
              <i class="fas fa-map-marker-alt" style="margin-right: 5px; color: #e74c3c;"></i>
    ...
    if (vendor.menus && vendor.menus.length > 0) {
      ...
      menuGrid.innerHTML = `
        <div style="text-align: center; padding: 40px; color: #7f8c8d;">
          <i class="fas fa-utensils" style="font-size: 48px; margin-bottom: 20px; color: #bdc3c7;"></i>
          <div style="font-size: 16px; margin-bottom: 10px;">Menu Belum Tersedia</div>
          <div style="font-size: 12px;">Hubungi penjual untuk informasi menu</div>
        </div>
      `;
    }
  ...
  // PERBAIKAN: Fungsi untuk membuka modal layanan antar
  ...
  // PERBAIKAN: Fungsi untuk menutup modal layanan antar
  ...
  function openMenuDetail(menuId, menuName, menuDesc, menuPrice, menuImage) {
    ...
    orderBtn.onclick = () => {
      if (currentVendor) {
        const message = `Halo, saya ingin memesan ${decodeURIComponent(menuName)} seharga Rp ${parseInt(menuPrice).toLocaleString('id-ID')}`;
